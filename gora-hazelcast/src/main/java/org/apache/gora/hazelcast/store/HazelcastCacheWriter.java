...
/**
 * {@link HazelcastCacheWriter} is the primary class
 * responsible for writing data beans to persistency dataStore from in memory cache.
 */
public class HazelcastCacheWriter<K, T extends PersistentBase> implements CacheWriter<K, T> {

  private static final Logger LOG = LoggerFactory.getLogger(HazelcastCacheWriter.class);
  private DataStore<K, T> dataStore;

  public HazelcastCacheWriter(DataStore<K, T> dataStore) {
