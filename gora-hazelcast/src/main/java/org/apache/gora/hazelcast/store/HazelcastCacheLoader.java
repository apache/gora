...
/**
 * {@link HazelcastCacheLoader} is the primary class
 * responsible for loading data beans from persistency dataStore to in memory cache.
 */
public class HazelcastCacheLoader<K, T extends PersistentBase> implements CacheLoader<K, T> {

  private static final Logger LOG = LoggerFactory.getLogger(HazelcastCacheLoader.class);
  private DataStore<K, T> dataStore;

  public HazelcastCacheLoader(DataStore<K, T> dataStore) {
