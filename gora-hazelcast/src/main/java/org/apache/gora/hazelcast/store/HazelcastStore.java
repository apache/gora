...
import org.apache.gora.hazelcast.query.HazelcastQuery;
import org.apache.gora.hazelcast.query.HazelcastResult;
...
/**
 * {@link HazelcastStore} is the primary class
...
public class HazelcastStore<K, T extends PersistentBase> extends DataStoreBase<K, T> {
  ...
  private static final String GORA_DEFAULT_JCACHE_PROVIDER_KEY = "gora.datastore.hazelcast.provider";
  private static final String HAZELCAST_READ_THROUGH_ENABLE = "hazelcast.read.through.enable";
  private static final String HAZELCAST_WRITE_THROUGH_ENABLE = "hazelcast.write.through.enable";
  private static final String HAZELCAST_STORE_BY_VALUE_ENABLE = "hazelcast.store.by.value.enable";
  private static final String HAZELCAST_STATISTICS_ENABLE = "hazelcast.statistics.enable";
  private static final String HAZELCAST_MANAGEMENT_ENABLE = "hazelcast.management.enable";
  private static final String HAZELCAST_EXPIRE_POLICY = "hazelcast.expire.policy";
  private static final String HAZELCAST_EXPIRE_POLICY_DURATION = "hazelcast.expire.policy.duration";
  private static final String HAZELCAST_ACCESSED_EXPIRY_IDENTIFIER = "ACCESSED";
  private static final String HAZELCAST_CREATED_EXPIRY_IDENTIFIER = "CREATED";
  private static final String HAZELCAST_MODIFIED_EXPIRY_IDENTIFIER = "MODIFIED";
  private static final String HAZELCAST_TOUCHED_EXPIRY_IDENTIFIER = "TOUCHED";
  private static final String HAZELCAST_AUTO_CREATE_CACHE_PROPERTY_KEY = "hazelcast.auto.create.cache";
  ...
  public void initialize(Class<K> keyClass, Class<T> persistentClass, Properties properties) throws GoraException {
    ...
    if (((properties.getProperty(HAZELCAST_AUTO_CREATE_CACHE_PROPERTY_KEY) != null) &&
            Boolean.valueOf(properties.getProperty(HAZELCAST_AUTO_CREATE_CACHE_PROPERTY_KEY)))
      ...
      if (properties.getProperty(HAZELCAST_READ_THROUGH_ENABLE) != null) {
        mutableCacheConfig.setReadThrough(Boolean.valueOf(properties.getProperty(HAZELCAST_READ_THROUGH_ENABLE)));
      } else {
        mutableCacheConfig.setReadThrough(true);
      }
      if (properties.getProperty(HAZELCAST_WRITE_THROUGH_ENABLE) != null) {
        mutableCacheConfig.setWriteThrough(Boolean.valueOf(properties.getProperty(HAZELCAST_WRITE_THROUGH_ENABLE)));
      } else {
        mutableCacheConfig.setWriteThrough(true);
      }
      if (properties.getProperty(HAZELCAST_STORE_BY_VALUE_ENABLE) != null) {
        mutableCacheConfig.setStoreByValue(Boolean.valueOf(properties.getProperty(HAZELCAST_STORE_BY_VALUE_ENABLE)));
      }
      if (properties.getProperty(HAZELCAST_STATISTICS_ENABLE) != null) {
        mutableCacheConfig.setStatisticsEnabled(Boolean.valueOf(properties.getProperty(HAZELCAST_STATISTICS_ENABLE)));
      }
      if (properties.getProperty(HAZELCAST_MANAGEMENT_ENABLE) != null) {
        mutableCacheConfig.setStatisticsEnabled(Boolean.valueOf(properties.getProperty(HAZELCAST_MANAGEMENT_ENABLE)));
      }
      if (properties.getProperty(HAZELCAST_EXPIRE_POLICY) != null) {
        String expiryPolicyIdentifier = properties.getProperty(HAZELCAST_EXPIRE_POLICY);
        if (expiryPolicyIdentifier.equals(HAZELCAST_ACCESSED_EXPIRY_IDENTIFIER)) {
          mutableCacheConfig.setExpiryPolicyFactory(FactoryBuilder.factoryOf(
                  new AccessedExpiryPolicy(new Duration(TimeUnit.SECONDS,
                          Integer.valueOf(properties.getProperty(HAZELCAST_EXPIRE_POLICY_DURATION))))
          ));
        } else if (expiryPolicyIdentifier.equals(HAZELCAST_CREATED_EXPIRY_IDENTIFIER)) {
          mutableCacheConfig.setExpiryPolicyFactory(FactoryBuilder.factoryOf(
                  new CreatedExpiryPolicy(new Duration(TimeUnit.SECONDS,
                          Integer.valueOf(properties.getProperty(HAZELCAST_EXPIRE_POLICY_DURATION))))
          ));
        } else if (expiryPolicyIdentifier.equals(HAZELCAST_MODIFIED_EXPIRY_IDENTIFIER)) {
          mutableCacheConfig.setExpiryPolicyFactory(FactoryBuilder.factoryOf(
                  new ModifiedExpiryPolicy(new Duration(TimeUnit.SECONDS,
                          Integer.valueOf(properties.getProperty(HAZELCAST_EXPIRE_POLICY_DURATION))))
          ));
        } else if (expiryPolicyIdentifier.equals(HAZELCAST_TOUCHED_EXPIRY_IDENTIFIER)) {
          mutableCacheConfig.setExpiryPolicyFactory(FactoryBuilder.factoryOf(
      ...
      mutableCacheConfig.setCacheLoaderFactory(HazelcastCacheFactoryBuilder
              .factoryOfCacheLoader(this.persistentDataStore, keyClass, persistentClass));
      mutableCacheConfig.setCacheWriterFactory(HazelcastCacheFactoryBuilder
    ...
    cache.registerCacheEntryListener(new MutableCacheEntryListenerConfiguration<>(
            HazelcastCacheFactoryBuilder
                    .factoryOfEntryListener(new HazelcastCacheEntryListener<K, T>(cacheEntryList)),
            null, true, true));
  ...
  public Result<K, T> execute(Query<K, T> query) throws GoraException {
    ...
    return new HazelcastResult<>(this, query, cacheEntrySubList);
  }

  @Override
  public Query<K, T> newQuery() {
    return new HazelcastQuery<>(this);
  }
