 ...
 */
public class RethinkDBMappingBuilder<K, T extends PersistentBase> {
  ...
  public static final Logger log = LoggerFactory.getLogger(RethinkDBMappingBuilder.class);

  private final RethinkDBStore<K, T> dataStore;

  private RethinkDBMapping mapping;

  public RethinkDBMappingBuilder(final RethinkDBStore<K, T> store) {
    this.dataStore = store;
    this.mapping = new RethinkDBMapping();
  }

  public RethinkDBMapping build() {
    if (mapping.getDocumentClass() == null)
      throw new IllegalStateException("Document Class is not specified.");
    return mapping;
  }

  protected RethinkDBMappingBuilder fromFile(String uri) throws IOException {
    try {
