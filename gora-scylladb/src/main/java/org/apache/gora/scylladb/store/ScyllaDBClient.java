...
/**
 * This class provides the ScyllaDB Client Connection.
 * Initialize the ScyllaDB Connection according to the Properties.
 */
public class ScyllaDBClient extends CassandraClient {

    private static final Logger LOG = LoggerFactory.getLogger(ScyllaDBClient.class);
    ...
    public void initialize(Properties properties, CassandraMapping mapping) throws Exception {
        ...
        readConsistencyLevel = properties.getProperty(ScyllaDBStoreParameters.READ_CONSISTENCY_LEVEL);
        writeConsistencyLevel = properties.getProperty(ScyllaDBStoreParameters.WRITE_CONSISTENCY_LEVEL);
    ...
    private TypeCodec getTypeCodec(String columnType) {
        ...
        switch (columnType) {
            ...
            case "text":
                ...
                break;
            default:
                LOG.error("Unsupported Scylla datatype: {} ", columnType);
                throw new RuntimeException("Unsupported Scylla datatype: " + columnType);
        }
        return typeCodec;
    }

    private Cluster.Builder populateSettings(Cluster.Builder builder, Properties properties) {
        String serversParam = properties.getProperty(ScyllaDBStoreParameters.SCYLLADB_SERVERS);
        String[] servers = serversParam.split(",");
        for (String server : servers) {
            builder = builder.addContactPoint(server);
        }
        String portProp = properties.getProperty(ScyllaDBStoreParameters.PORT);
        if (portProp != null) {
            builder = builder.withPort(Integer.parseInt(portProp));
        }
        String clusterNameProp = properties.getProperty(ScyllaDBStoreParameters.CLUSTER_NAME);
        if (clusterNameProp != null) {
            builder = builder.withClusterName(clusterNameProp);
        }
        String compressionProp = properties.getProperty(ScyllaDBStoreParameters.COMPRESSION);
        ...
        String enableJMXProp = properties.getProperty(ScyllaDBStoreParameters.ENABLE_JMX_REPORTING);
        if (!Boolean.parseBoolean(enableJMXProp)) {
            builder = builder.withoutJMXReporting();
        }
        String enableMetricsProp = properties.getProperty(ScyllaDBStoreParameters.ENABLE_METRICS);
        ...
        String versionProp = properties.getProperty(ScyllaDBStoreParameters.PROTOCOL_VERSION);
        if (versionProp != null) {
        ...
        String enableSSLProp = properties.getProperty(ScyllaDBStoreParameters.ENABLE_SSL);
    ...
    private Cluster.Builder populateLoadBalancingProp(Properties properties, Cluster.Builder builder) {
        String loadBalancingProp = properties.getProperty(ScyllaDBStoreParameters.LOAD_BALANCING_POLICY);
        if (loadBalancingProp != null) {
            switch (loadBalancingProp) {
                ...
                case "DCAwareRoundRobinPolicy": {
                    String dataCenter = properties.getProperty(ScyllaDBStoreParameters.DATA_CENTER);
                    boolean allowRemoteDCsForLocalConsistencyLevel = Boolean.parseBoolean(
                            properties.getProperty(ScyllaDBStoreParameters.ALLOW_REMOTE_DCS_FOR_LOCAL_CONSISTENCY_LEVEL));
                ...
                default:
                    LOG.error("Unsupported ScyllaDB load balancing  policy: {} ", loadBalancingProp);
                    break;
            }
        }
        return builder;
    ...
    private Cluster.Builder populateCredentials(Properties properties, Cluster.Builder builder) {
        String usernameProp = properties.getProperty(ScyllaDBStoreParameters.USERNAME);
        String passwordProp = properties.getProperty(ScyllaDBStoreParameters.PASSWORD);
    ...
    private Cluster.Builder populatePoolingSettings(Properties properties, Cluster.Builder builder) {
        String localCoreConnectionsPerHost = properties.getProperty(ScyllaDBStoreParameters.LOCAL_CORE_CONNECTIONS_PER_HOST);
        String remoteCoreConnectionsPerHost = properties.getProperty(ScyllaDBStoreParameters.REMOTE_CORE_CONNECTIONS_PER_HOST);
        String localMaxConnectionsPerHost = properties.getProperty(ScyllaDBStoreParameters.LOCAL_MAX_CONNECTIONS_PER_HOST);
        String remoteMaxConnectionsPerHost = properties.getProperty(ScyllaDBStoreParameters.REMOTE_MAX_CONNECTIONS_PER_HOST);
        String localNewConnectionThreshold = properties.getProperty(ScyllaDBStoreParameters.LOCAL_NEW_CONNECTION_THRESHOLD);
        String remoteNewConnectionThreshold = properties.getProperty(ScyllaDBStoreParameters.REMOTE_NEW_CONNECTION_THRESHOLD);
        String localMaxRequestsPerConnection = properties.getProperty(ScyllaDBStoreParameters.LOCAL_MAX_REQUESTS_PER_CONNECTION);
        String remoteMaxRequestsPerConnection = properties.getProperty(ScyllaDBStoreParameters.REMOTE_MAX_REQUESTS_PER_CONNECTION);
    ...
    private Cluster.Builder populateQueryOptions(Properties properties, Cluster.Builder builder) {
        String consistencyLevelProp = properties.getProperty(ScyllaDBStoreParameters.CONSISTENCY_LEVEL);
        String serialConsistencyLevelProp = properties.getProperty(ScyllaDBStoreParameters.SERIAL_CONSISTENCY_LEVEL);
        String fetchSize = properties.getProperty(ScyllaDBStoreParameters.FETCH_SIZE);
    ...
    private Cluster.Builder populateReconnectPolicy(Properties properties, Cluster.Builder builder) {
        String reconnectionPolicy = properties.getProperty(ScyllaDBStoreParameters.RECONNECTION_POLICY);
        if (reconnectionPolicy != null) {
            switch (reconnectionPolicy) {
                case "ConstantReconnectionPolicy": {
                    String constantReconnectionPolicyDelay = properties.getProperty(ScyllaDBStoreParameters.CONSTANT_RECONNECTION_POLICY_DELAY);
                ...
                case "ExponentialReconnectionPolicy": {
                    String exponentialReconnectionPolicyBaseDelay = properties.getProperty(ScyllaDBStoreParameters.EXPONENTIAL_RECONNECTION_POLICY_BASE_DELAY);
                    String exponentialReconnectionPolicyMaxDelay = properties.getProperty(ScyllaDBStoreParameters.EXPONENTIAL_RECONNECTION_POLICY_MAX_DELAY);
    ...
    private Cluster.Builder populateRetrytPolicy(Properties properties, Cluster.Builder builder) {
        String retryPolicy = properties.getProperty(ScyllaDBStoreParameters.RETRY_POLICY);
        if (retryPolicy != null) {
            switch (retryPolicy) {
    ...
    private Cluster.Builder populateSocketOptions(Properties properties, Cluster.Builder builder) {
        String connectionTimeoutMillisProp = properties.getProperty(ScyllaDBStoreParameters.CONNECTION_TIMEOUT_MILLIS);
        String keepAliveProp = properties.getProperty(ScyllaDBStoreParameters.KEEP_ALIVE);
        String readTimeoutMillisProp = properties.getProperty(ScyllaDBStoreParameters.READ_TIMEOUT_MILLIS);
        String receiveBufferSizeProp = properties.getProperty(ScyllaDBStoreParameters.RECEIVER_BUFFER_SIZE);
        String reuseAddress = properties.getProperty(ScyllaDBStoreParameters.REUSE_ADDRESS);
        String sendBufferSize = properties.getProperty(ScyllaDBStoreParameters.SEND_BUFFER_SIZE);
        String soLinger = properties.getProperty(ScyllaDBStoreParameters.SO_LINGER);
        String tcpNoDelay = properties.getProperty(ScyllaDBStoreParameters.TCP_NODELAY);
    ...
    private List<String> readCustomCodec(Properties properties) throws JDOMException, IOException {
        String filename = properties.getProperty(ScyllaDBStoreParameters.CUSTOM_CODEC_FILE);
        if (filename != null) {
