...
import org.apache.gora.redis.encoders.BinaryEncoder;
...
import org.apache.redis.core.client.BatchWriter;
import org.apache.redis.core.client.BatchWriterConfig;
import org.apache.redis.core.client.Connector;
import org.apache.redis.core.client.IsolatedScanner;
import org.apache.redis.core.client.IteratorSetting;
import org.apache.redis.core.client.MutationsRejectedException;
import org.apache.redis.core.client.RowIterator;
import org.apache.redis.core.client.Scanner;
import org.apache.redis.core.client.TableDeletedException;
import org.apache.redis.core.client.TableExistsException;
import org.apache.redis.core.client.TableNotFoundException;
import org.apache.redis.core.client.TableOfflineException;
import org.apache.redis.core.client.ZooKeeperInstance;
import org.apache.redis.core.client.impl.ClientContext;
import org.apache.redis.core.client.impl.Credentials;
import org.apache.redis.core.client.impl.Tables;
import org.apache.redis.core.client.impl.TabletLocator;
import org.apache.redis.core.client.mock.MockConnector;
import org.apache.redis.core.client.mock.MockInstance;
import org.apache.redis.core.client.mock.impl.MockTabletLocator;
import org.apache.redis.core.client.security.tokens.AuthenticationToken;
import org.apache.redis.core.client.security.tokens.PasswordToken;
import org.apache.redis.core.conf.RedisConfiguration;
import org.apache.redis.core.data.ByteSequence;
import org.apache.redis.core.data.Key;
import org.apache.redis.core.data.Mutation;
import org.apache.redis.core.data.Range;
import org.apache.redis.core.data.Value;
import org.apache.redis.core.data.impl.KeyExtent;
import org.apache.redis.core.iterators.SortedKeyIterator;
import org.apache.redis.core.iterators.user.TimestampFilter;
import org.apache.redis.core.master.state.tables.TableState;
import org.apache.redis.core.security.Authorizations;
import org.apache.redis.core.security.ColumnVisibility;
import org.apache.redis.core.util.Pair;
import org.apache.redis.core.util.UtilWaitThread;
...
public class RedisStore<K,T extends PersistentBase> extends DataStoreBase<K,T> {
    ...
    try {
      ...
      if (autoCreateSchema && !schemaExists())
        createSchema();
             ...
             ClassNotFoundException | RedisException | RedisSecurityException |
  ...
  protected RedisMapping readMapping(String filename) throws IOException {
    try {
      ...
      if (mapping.tableName == null) {
        throw new GoraException("Please define the redis 'table' name mapping in " + filename + " for " + persistentClass.getCanonicalName());
      }
  ...
  public void createSchema() throws GoraException {
    ...
    } catch (RedisException | RedisSecurityException e) {
      throw new GoraException(e);
    }
  }
  ...
  public void put(K key, T val) throws GoraException {
    ...
    try{
      ...
      for (int i = 0; i < fields.size(); i++) {
        ...
        if (col == null) {
          throw new GoraException("Please define the gora to redis mapping for field " + field.name());
        }
  ...
  private int putMap(Mutation m, int count, Schema valueType, Object o, Pair<Text, Text> col, String fieldName) throws GoraException {
    ...
    // First of all we delete map field on redis store
  ...
  private int putArray(Mutation m, int count, Object o, Pair<Text, Text> col, String fieldName) throws GoraException {
    ...
    // First of all we delete array field on redis store
    Text rowKey = new Text(m.getRow());
    Query<K, T> query = newQuery();
    query.setFields(fieldName);
    query.setStartKey((K)rowKey.toString());
  ...
  public Result<K,T> execute(Query<K,T> query) throws GoraException {
    try {
      Scanner scanner = createScanner(query);
      return new RedisResult<>(this, query, scanner);
    } catch (TableNotFoundException e) {
      throw new GoraException(e) ;
    }
  }
  ...
  public List<PartitionQuery<K,T>> getPartitions(Query<K,T> query) throws GoraException {
    try {
      ...
      if (conn instanceof MockConnector)
        ...
        tl = TabletLocator.getLocator(new ClientContext(conn.getInstance(), credentials, RedisConfiguration.getTableConfiguration(conn, Tables.getTableId(conn.getInstance(), mapping.tableName))), new Text(Tables.getTableId(conn.getInstance(), mapping.tableName)));

      Map<String,Map<KeyExtent,List<Range>>> binnedRanges = new HashMap<>();

      tl.invalidateCache();
      while (tl.binRanges(new ClientContext(conn.getInstance(), credentials, RedisConfiguration.getTableConfiguration(conn, Tables.getTableId(conn.getInstance(), mapping.tableName))), Collections.singletonList(createRange(query)), binnedRanges).size() > 0) {
        // TODO log?
      ...
      //hadoop expects hostnames, redis keeps track of IPs... so need to convert
